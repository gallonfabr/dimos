...
"""Tests for GStreamer-based VU meter operator."""
...
def test_gst_vumeter_passthrough():
    """Test that GStreamer VU meter passes audio through unchanged."""
    ...
    signal(
        ...
        vumeter(bar_length=30),
        ops.do_action(on_next=on_next, on_completed=on_completed),
    ).run()

    assert event_count > 0, "Expected events from GStreamer VU meter"
...
def test_gst_vumeter_with_rms():
    """Test GStreamer VU meter with RMS volume calculation."""
    ...
    signal(
        ...
        vumeter(use_rms=True),
        ops.do_action(on_next=on_next),
    ).run()

    assert event_count > 0, "Expected events from RMS GStreamer VU meter"

    # Give cleanup threads time to finish
    time.sleep(0.2)


def test_gst_vumeter_minimal():
    """Test GStreamer VU meter with minimal display (no percentage or activity)."""
    ...
    assert event_count > 0, "Expected events from minimal GStreamer VU meter"
