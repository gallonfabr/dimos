...
"""G1 with PGO (pose graph optimization) on real hardware.

Adds loop-closure-corrected mapping on top of the base SmartNav navigation
stack. PGO accumulates registered scans as keyframes and runs iSAM2 loop
closure to produce a globally consistent global map. The corrected_odometry
output can be monitored in Rerun for drift comparison.

Data flow:
    FastLio2 → registered_scan + odometry
    → PGO → corrected_odometry (visualization) + global_map (accumulated)
    FastLio2.odometry → SensorScanGeneration → TerrainAnalysis → LocalPlanner
    → PathFollower → G1HighLevelDdsSdk
"""
...
unitree_g1_nav_pgo_onboard = (
    autoconnect(
        FastLio2.blueprint(
            ...
            map_freq=0.0,  # PGO provides the global map
        ),
        PGO.blueprint(),
...
def main() -> None:
    unitree_g1_nav_pgo_onboard.build().loop()


__all__ = ["unitree_g1_nav_pgo_onboard"]
